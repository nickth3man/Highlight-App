...
def load_api_keys(env_path: str | None = None) -> APIKeys:
    ...
    masked_youtube = (
        "*" * 8 + keys.youtube_key[-4:]
        if keys.youtube_key and len(keys.youtube_key) > 4
        else "None"
    )
    masked_twitter = (
        "*" * 8 + keys.twitter_api_key[-4:]
        if keys.twitter_api_key and len(keys.twitter_api_key) > 4
        else "None"
    )
    ...
    if keys.youtube_key and keys.youtube_key.startswith("your_"):
        logger.warning(
            "YouTube API key appears to be a placeholder (starts with 'your_')"
        )
    if keys.twitter_api_key and keys.twitter_api_key.startswith("your_"):
        logger.warning(
            "Twitter API key appears to be a placeholder (starts with 'your_')"
        )
